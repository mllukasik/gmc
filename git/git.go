...
func (repo *Repository) CurrentBranch() (string, error) {
	...
	return "", GMCGitError{message: "Could not determine current branch"}
}
...
func (repo *Repository) Checkout(branch Branch) error {
	...
	err := cmd.Run()
	if stderr.Len() != 0 {
		return GMCGitError{message: stderr.String()}
	}
	return err
}
...
func (repo *Repository) getDefaultRemote(value *string) (string, error) {
	...
	if len(remotes) != 1 {
		return "", GMCGitError{message: "There is no exactly one remote defined. Please specify remote"}
	}
...
type GMCGitError struct {
	message string
}

func (err GMCGitError) Error() string {
	return err.message
}
